...
<div class="px-2">
	<div class="flex h-16 items-center justify-between px-2">
		<div class="item-center relative my-[20px] flex justify-center">
			<Search class="absolute bottom-2 left-2" />
			<Input class="absolute; box-sizing: border-box w-64 pl-[40px]" placeholder="搜索" />
		</div>
		<div>
			<Dialog.Root>
				<Dialog.Trigger class={buttonVariants({ variant: 'outline' })}>
					<Plus size="15px" />
					<p class="text-xs font-semibold">添加导入</p>
				</Dialog.Trigger>
				<Dialog.Content class="sm:max-w-[550px]">
					...
					<form class="h-[450px]">
						...
						<div class="my-4 flex flex-col space-y-1.5">
	...
	<div class="h-3/4 w-full border border-gray-200 rounded-md">
		<ScrollArea class="h-[610px] w-full rounded-md ">
			<Table.Root>
				<Table.Header class="">
					<Table.Row>
					...
					{#each invoices as invoice, i (i)}
						<Table.Row>
							<Table.Cell class="font-medium "
								><p class="... w-4/5 truncate">{invoice.invoice}</p></Table.Cell
							>
							<Table.Cell class="font-medium"
								><p class="... w-4/5 truncate">{invoice.answer}</p></Table.Cell
							>
	...
	<div class="mr-4 flex justify-end pt-2 mt-2">
