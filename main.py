...
from apps.llama.main import app as llama_app
...
print(
    f"""
 __     ___       _    _      __  __       ______      
 \ \   / (_)     | |  | |    |  \/  |     |  ____|     
  \ \_/ / _ _ __ | |__| | ___| \  / | ___ | |__   _ __ 
   \   / | | '_ \|  __  |/ _ \ |\/| |/ _ \|  __| | '__|
    | |  | | | | | |  | |  __/ |  | | (_) | |____| |   
    |_|  |_|_| |_|_|  |_|\___|_|  |_|\___/|______|_|  
...
def process_data():
    # 你的数据库连接信息和操作
    metadata_base = 'dataset'

    # df 传值后续从前端接收
    df = generate_dataset()
    create_command = dataset_create_table_sql()
    cnx = connect(user='knowledge_base', password='N4#w!7pLxQ', host='localhost', database='tmp', port=3309)

    try:
        # 执行函数，传入建表语句和数据
        execute_with_dynamic_insert(cnx, create_command, df, metadata_base, 'tmp')
        return {"message": "数据处理成功"}
    finally:
        # 关闭数据库连接
        cnx.close()
        print('脚本执行完成，关闭连接')
